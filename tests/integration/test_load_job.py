...
def test_batch_write_top_songs(ddb_tables):
    ...
    resp = table.query(
        KeyConditionExpression=Key("genre").eq("rock") & Key("date_rank").begins_with("2024-06-25#")
    )
...
def test_idempotent_overwrite(ddb_tables):
    ...
    item1 = shape_for_dynamo(
        "genre_daily",
        {"genre": "pop", "listen_date": "2024-06-25", "listen_count": 100,
         "unique_listeners": 50, "total_listening_time_ms": 5_000_000,
    ...
    scan = table.scan(FilterExpression="genre = :g", ExpressionAttributeValues={":g": "pop"})
    assert scan["Count"] == 1
